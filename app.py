...
def home():
    ...
    return (
        "🚀 El puente cuantitativo MES Quant V5 con ejecución directa en Tradovate (Demo/Validación) está activo.",
        200,
    )
...
def ejecutar_orden_tradovate(accion, volumen, simbolo="MESZ6"):
    """Conexión directa con la URL base de Tradovate (Entorno Demo/Simulación validado)."""
    ...
    # URL base ajustada al entorno Demo donde opera físicamente la cuenta a nivel API
    ...
    try:
        ...
        if order_response.status_code == 200:
            print(f"✅ ¡Orden ejecutada con éxito en Tradovate (Demo/Validación)!: {order_response.json()}")
            return True
        else:
...
def procesar_tarea_segundo_plano(datos, tiempo_inicio):
    ...
    try:
        ...
        # 2. Ejecución real en el Bróker (Tradovate Demo/Validación)
